    ...
    if (!princess && !fatloser) {
        ...
        if (mylains) {
            if !throweeobj {
                ...
                object_event_add(throweeobj,ev_create,0,"alpha=1 visible=1 name=owner.o.name p2=owner.o.p2 size=owner.o.size orig=owner.o is_spriteswitcher=1 sprite='stand' oldspr='stand' charm_init() sheet=owner.o.sheets[owner.o.size] gravity=0 frame=0 frspd=0 xsc=1 ysc=1")
                object_event_add(throweeobj,ev_step,ev_step_end,"spriteswitch(1)")
                object_event_add(throweeobj,ev_draw,0,"if owner.usepalette scr_applyPaletteSegmented(global.shaderPaletteSwap,global.palettesprites[p2*100],global.pal_1[p2]+1,global.pal_2[p2]+1,global.pal_3[p2]+1,global.pal_4[p2]+1,size,totpal+1) spriteswitch(0)")
            }
        }
    } else depth=-1
}
...
if (mylains) {
    ...
    if (throwee.thrown) {
        ...
        with (throwee) {if (xsc>=10 || ysc>=10) instance_destroy() object_delete(throweeobj) throweeobj=-1 throwee=noone}
    }
} else if (yare) {
